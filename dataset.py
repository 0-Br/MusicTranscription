...
class MidiMixIterDataset(IterableDataset):
    ...
    def __init__(
        ...
        self.df = self._build_dataset(root_dir, split_key=split)
    ...
    def _build_dataset(self, root_dir, split_key='train', shuffle=True):
        df = []
        json_file = glob(f'{root_dir}/*.json')
        assert len(json_file) == 1
        df = pd.read_json(json_file[0])

        # transform to list of dicts
        df_train = df[df['split'] == 'train'].to_dict('records')
        df_test = df[df['split'] == 'test'].to_dict('records')
        df_valid = df[df['split'] == 'validation'].to_dict('records')

        if shuffle:
            random.shuffle(df_train)
        df_all = {'train': df_train, 'test': df_test, 'validation': df_valid}
        return df_all[split_key]
    ...
    def _parse_midi(self, path, instrument_dict: Dict[str, str]):
        ...
        for filename in instrument_dict.keys():
            midi_path = f'{self.root_dir}/{path}'
            note_seqs.append(note_seq.midi_file_to_note_sequence(midi_path))
    ...
    def __iter__(self):
        for idx in range(len(self.df)):
            ...
            audio, sr = librosa.load(os.path.join(self.root_dir, row['audio_path']), sr=None)
...
if __name__ == '__main__':
    dataset = MidiMixIterDataset('data', "train")
