...
class InferenceHandler:
    ...
    #choose specific program(instruments)
    ...
    def _postprocess_batch(self, result):
        ...
        result = torch.where(after_eos.bool(), -1, result)  # -1 = DECODED_EOS_ID
        # remove bos
        result = result[:, 1:]
    ...
    def _to_event(self, predictions_np: List[np.ndarray], frame_times: np.ndarray):
        ...
        for i, batch in enumerate(predictions_np):
            for j, tokens in enumerate(batch):
                ...
                start_time -= start_time % (1 / self.codec.steps_per_second)  #使得时间分辨率一致
                predictions.append({
                    'est_tokens': tokens,
...
if __name__ == "__main__":
    ...
    handler.inference('test_data/S05.wav')